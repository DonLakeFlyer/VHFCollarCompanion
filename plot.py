...
class Waterfall(object):
    ...
    def start(self):
        ...
        curMag, freqs = magnitude_spectrum(samples, Fs=self.sdr.rs)
        lastMax = max(curMag)
        line1, = self.ax.plot(curMag)
        while True:
            self.fig.canvas.draw()
            samples = self.sdr.read_samples(1024)
            newMag, freqs = magnitude_spectrum(samples, Fs=self.sdr.rs)
            for i in range(len(newMag)):
                curMag[i] *= 0.75
                curMag[i] = max(curMag[i] * 0.75, newMag[i])
            line1.set_ydata(curMag)
            lastMax = max(lastMax, max(curMag))
            plt.ylim(ymax=lastMax)
        return
